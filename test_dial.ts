...
async function main() {
    ...
    const client = new TeleQuickClient(
        "quic://187.127.139.138:9090",
        "dummy_sa.json",
        false,      // Not a browser token
    ...
    try {
        ...
        const active_calls = new Set<string>();

        client.onAudioFrame = (payload: Uint8Array) => {
            try {
        ...
        client.onCallEvent = (payload: Uint8Array) => {
            try {
                if (client.wasmModule) {
